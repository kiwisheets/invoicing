...
func (ec *executionContext) __resolve_entities(ctx context.Context, representations []map[string]interface{}) ([]fedruntime.Entity, error) {
	...
	for _, rep := range representations {
		...
		case "Client":
			id0, err := ec.unmarshalNID2githubᚗcomᚋemviᚋhideᚐID(ctx, rep["id"])
			if err != nil {
				return nil, errors.New(fmt.Sprintf("Field %s undefined in schema.", "id"))
			}

			entity, err := ec.resolvers.Entity().FindClientByID(ctx,
				id0)
			if err != nil {
				return nil, err
			}
		...
		case "Company":
			...
			entity, err := ec.resolvers.Entity().FindCompanyByID(ctx,
				id0)
			if err != nil {
				return nil, err
			}
